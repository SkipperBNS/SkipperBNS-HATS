...
try {
$ConfigText = Get-Content -LiteralPath $ComponentsFile -Raw -ErrorAction Stop
...
try {
    Invoke-WebRequest -Uri $Asset.browser_download_url -OutFile $Destination -Headers @{ "User-Agent" = "SkipperBNS-HATS-Builder" } -ErrorAction Stop
}
catch {
...
if ($Component.mode -eq "zip") {
    ...
    Expand-Archive -LiteralPath $DownloadPath -DestinationPath $Pack -Force
...
# ========================================
# NRO / OVL / FILE COMPONENT
# ========================================

if ($Component.mode -eq "nro" -or $Component.mode -eq "ovl" -or $Component.mode -eq "file") {
...
$PackFiles = @(Get-ChildItem -LiteralPath $Pack -File -Recurse -ErrorAction Stop)
...
$RelativePath = $File.FullName.Substring($Pack.Length).TrimStart("")
...
Compress-Archive -Path "$Pack*" -DestinationPath $Output -Force
